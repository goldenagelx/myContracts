...
namespace phonebook
{
    public partial class Form_Main : Form
    {
        ...
        {
            if (MessageBox.Show("确认离开通讯录？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
            {
                this.Close();
            }
            else
        ...
        private void treeView1_AfterSelect_1(object sender, TreeViewEventArgs e)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                this.dataGridView1.BindingContext[this.dataGridView1.DataSource].SuspendBinding();
                this.dataGridView1.Rows[i].Visible = true;
                this.dataGridView1.BindingContext[this.dataGridView1.DataSource].ResumeBinding();
        ...
        private void treeView1_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            if (name == "1")
            {
                for (int i = 0; i < num; i++)
                {
                    if (dataGridView1.Rows[i].Cells[8].Value.ToString() == "计算机科学与技术")
                    {
                        this.dataGridView1.BindingContext[this.dataGridView1.DataSource].SuspendBinding();
                        this.dataGridView1.Rows[i].Visible = true;
                        this.dataGridView1.BindingContext[this.dataGridView1.DataSource].ResumeBinding();
            ...
            if (name == "2")
            {
                for (int i = 0; i < num; i++)
                {
                    if (dataGridView1.Rows[i].Cells[8].Value.ToString() == "信息科学与技术")
                    {
                        this.dataGridView1.BindingContext[this.dataGridView1.DataSource].SuspendBinding();
                        this.dataGridView1.Rows[i].Visible = true;
                        this.dataGridView1.BindingContext[this.dataGridView1.DataSource].ResumeBinding();
            ...
            if (name == "3")
            {
                for (int i = 0; i < num; i++)
                {
                    if (dataGridView1.Rows[i].Cells[8].Value.ToString() == "信息安全与网络工程")
                    {
                        this.dataGridView1.BindingContext[this.dataGridView1.DataSource].SuspendBinding();
                        this.dataGridView1.Rows[i].Visible = true;
                        this.dataGridView1.BindingContext[this.dataGridView1.DataSource].ResumeBinding();
            ...
            if (name == "4")
            {
                for (int i = 0; i < num; i++)
                {
                    if (dataGridView1.Rows[i].Cells[8].Value.ToString() == "人工智能")
                    {
                        this.dataGridView1.BindingContext[this.dataGridView1.DataSource].SuspendBinding();
                        this.dataGridView1.Rows[i].Visible = true;
                        this.dataGridView1.BindingContext[this.dataGridView1.DataSource].ResumeBinding();
            ...
            if (name == "5")
            {
                for (int i = 0; i < num; i++)
                {
                    if (dataGridView1.Rows[i].Cells[8].Value.ToString() == "大数据")
                    {
                        this.dataGridView1.BindingContext[this.dataGridView1.DataSource].SuspendBinding();
                        this.dataGridView1.Rows[i].Visible = true;
                        this.dataGridView1.BindingContext[this.dataGridView1.DataSource].ResumeBinding();
